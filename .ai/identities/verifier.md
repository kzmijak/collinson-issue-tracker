...
- The things that the ACCS must check are dictated by the accs.md, not the contract. Contract is helpful for implementing the ACCS from accs.md, but it must not provide additional required checks. If it does introduces new checks, it should be treated as optional, not mandatory. 